...
### Grafana и тестовое приложение будет работать на 80 порту. Для маршрутизации трафика между pods по 80 порту создаем proxy, который на основе префикса будет направлять трафик. На сервис тестового приложение префикс будет diplom, а для Grafana префикс будет grafana. В прокси делаем ingress 80 порта.
